...
def plot_decay(results, fill=True, titles=None, means=None):
    ...
    # for res, ax, title, mean in zip(results[3:], axes[1], titles[3:], means):
    #     plot_prf(ax, res, 0.3, neg=True)
    #     plot_prf(ax, res)
    #     # if mean is not None:
    #     #     plot_cv_mean(ax, mean)
...
def set_style():
    ...
    sns.set('paper', font='serif', style='ticks')
    # , rc={
    #     'font.family': 'serif',
    ...
    for i, ax in enumerate(axes):
        ...
        if periods > 12:
            labels = [str(x + 1) if x % 3 == 0
                      else '' for x in range(periods)]  #PRIMA ERA periods + 1 nel range
        else:
            labels = [str(x + 1) for x in range(periods)]   #idem qui
