...
def get_train_info(y_train, existing=None):
    ...
    results['train_neg'].append(y_train.shape[0] - train_pos)
    results['train_tot'].append(y_train.shape[0])
...
def print_metrics(results, filename=None, keys=None, header=True):
    f = None
    ...
    def print_cond(f, s):
        if f is not None:
            ...
            print(s)
        # print(s) if f is not None else f.write(s)

    def print_hline(f):
        print_cond(f, ('-' * 12) + '+' + ('-' * 7 * periods))
    ...
    print_cond(f, "AUT(F1): " + str(aut(results, 'f1')))
