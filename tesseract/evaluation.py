...
def fit_predict_update(clf, X_train, X_tests,
                       ...
                       retrain=False, pretrained=False,
    ...
    # QUI è TRAPIANTATO PER TRAINARE UNA VOLTA SOLA PRIMA DI TESTARE
    # (TANTO NON FACCIO RETRAINING)
    if isinstance(clf, CClassifier):
    ...
    for i, (X_test, y_test, t_test) in tqdm(enumerate(
        ...
        if retrain:
            if i >= 1:
                X_train = scipy.sparse.vstack((X_train, X_tests[i-1]))
                y_train = np.concatenate((y_train, y_tests[i-1]))
                t_train = np.concatenate((t_train, t_tests[i-1]))
                fit_function(X_train, y_train)
                print("")
        ...
        results = metrics.get_train_info(y_train, existing=results)
        ...
        y_pred = predict_function(X_test)   #unica riga originale

        if not isinstance(clf, RandomForestClassifier):
            score = clf.decision_function(X_test)
            scores.append(score[:, 1].flatten().tondarray() if isinstance(clf, CClassifier) else score)
        else:
            score = clf.predict_proba(X_test)
            scores.append(score[:, 1].flatten())
    ...
    return results, scores, y_tests, t_tests
