...
def main_train_sequence():
    ...
    # val_size = [0, 1, 2]
    # max_fpr = [0.01, 0.02, 0.03]
    # prod = list(cartesian_product(C, class_weight, val_size, max_fpr))
    # n_runs = len(prod)
    # for i, (C, class_weight, val_size, max_fpr) in enumerate(prod):
    #     print(f"{i+1}/{n_runs}: C: {C}, class_weight: {class_weight}, val_size: {val_size}, max_fpr: {max_fpr}")
